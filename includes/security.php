 ...
 * 3. Session Hardening (Strict Mode, HttpOnly, SameSite, Proxy-aware SSL)
...
// 3. Inisialisasi Sesi Aman (Secure & Proxy-Aware Session Init)
...
if (!defined('DB_NAME')) define('DB_NAME', env('DB_DATABASE', 'warkop_madam'));
...
function get_db_connection() {
    ...
    if ($pdo === null) {
        ...
        try {
            ...
            if ($app_debug) {
                die("Koneksi Database Gagal: " . e($e->getMessage()));
            } else {
                die("<!DOCTYPE html><html lang='id'><head><meta charset='UTF-8'><title>Koneksi Database Terputus</title><style>body{background:#09090b;color:#fca5a5;font-family:sans-serif;display:flex;align-items:center;justify-content:center;height:100vh;margin:0;}.card{background:#18181b;padding:28px;border-radius:16px;border:1px solid #7f1d1d;text-align:center;max-width:440px;box-shadow:0 20px 25px -5px rgba(0,0,0,0.5);}h2{color:#ef4444;margin-top:0;}p{color:#d4d4d8;font-size:14px;line-height:1.6;}a{display:inline-block;margin-top:16px;background:#d97706;color:#fff;padding:8px 18px;border-radius:8px;text-decoration:none;font-weight:600;font-size:13px;}</style></head><body><div class='card'><h2>⚠️ Gangguan Koneksi Database</h2><p>Sistem tidak dapat terhubung ke database. Pastikan file <b>.env</b> telah dikonfigurasi dengan benar atau jalankan inisialisasi database.</p><a href='db_init.php'>Cek / Inisialisasi Database</a></div></body></html>");
            }
        }
    }
    return $pdo;
...
function validate_and_save_upload($file_array, $target_dir = 'uploads/proofs/', $max_size_bytes = null) {
    ...
    // Verifikasi MIME Type aktual menggunakan fileinfo jika tersedia
    ...
    // Pastikan path target absolut atau relatif terhadap root proyek
    $project_root = dirname(__DIR__);
    $normalized_target_dir = $target_dir;
    if (strpos($target_dir, '/') !== 0 && strpos($target_dir, ':') !== 1) {
        $absolute_target_dir = $project_root . '/' . ltrim($target_dir, '/');
    } else {
        $absolute_target_dir = $target_dir;
    }
    ...
    $destination = rtrim($absolute_target_dir, '/') . '/' . $safe_name;
    $relative_path = rtrim($target_dir, '/') . '/' . $safe_name;
