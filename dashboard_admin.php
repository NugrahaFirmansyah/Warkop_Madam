...
<body class="bg-[#09090b] text-stone-100 min-h-screen flex selection:bg-amber-500 selection:text-stone-950">
    ...
    $pending_badge = $stats['pending_orders'];
                                                ...
                                                <td class="py-3 px-4 whitespace-nowrap">
                                                    <?php if (!empty($row['payment_proof'])): ?>
                                                        <div class="flex flex-col gap-1 items-start">
                                                            <button onclick="viewPaymentProofModal(<?php echo htmlspecialchars(json_encode($row), ENT_QUOTES, 'UTF-8'); ?>)" class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-lg bg-amber-500/15 hover:bg-amber-500/25 border border-amber-500/30 text-amber-300 text-[10px] font-bold transition">
                                                                <i class="fa-solid fa-receipt"></i>
                                                            ...
                                                            <?php if (($row['ai_status'] ?? '') === 'valid'): ?>
                                                                <span class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[8px] font-bold bg-emerald-500/20 text-emerald-300 border border-emerald-500/30">
                                                                    <i class="fa-solid fa-shield-check text-[8px]"></i> AI Lolos (<?php echo intval($row['ai_confidence'] ?? 95); ?>%)
                                                                </span>
                                                            <?php elseif (($row['ai_status'] ?? '') === 'review'): ?>
                                                                <span class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[8px] font-bold bg-amber-500/20 text-amber-300 border border-amber-500/30 animate-pulse">
                                                                    <i class="fa-solid fa-triangle-exclamation text-[8px]"></i> AI Perlu Cek
                                                                </span>
                                                            <?php elseif (($row['ai_status'] ?? '') === 'invalid'): ?>
                                                                <span class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[8px] font-bold bg-red-500/20 text-red-300 border border-red-500/30">
                                                                    <i class="fa-solid fa-circle-xmark text-[8px]"></i> AI Ditolak
                                                                </span>
                                                            <?php endif; ?>
                                                        </div>
                                                    <?php else: ?>
